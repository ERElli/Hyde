...
Level Class: the Level class is responsible for all things contained in the level. In each level, the player is able to select a background, different types of terrain in the form of Blocks(either a building or sidewalk made from 31 by pixel squares), checkpoints/weapons/enemies throughout the level and a soundtrack. The methods of this class are built around the player setting the x and y positions of objects like checkpoints, weapons, blocks and enemies and also setting the background and soundtrack.
...
Enemy Class: also referenced in the entity class description, gives properties and behaviour of various enemies including bosses 
